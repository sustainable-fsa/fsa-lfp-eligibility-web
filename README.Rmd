...
This repository is the web-published companion to [`sustainable-fsa/fsa-lfp-eligibility`](https://github.com/sustainable-fsa/fsa-lfp-eligibility), which archives the same LFP county eligibility data as obtained directly from FSA via FOIA request. The two datasets share a common schema and are directly comparable.
...
LFP provides compensation to eligible livestock producers who suffer grazing losses on drought-afflicted pastureland. Eligibility is determined weekly, by county and pasture type, from the [U.S. Drought Monitor](https://droughtmonitor.unl.edu/). Payment rates depend on drought severity and duration during the normal grazing period:

- **D2 (Severe Drought)** for at least four consecutive weeks — *one* monthly payment
- **D2 (Severe Drought)** for at least seven out of eight consecutive weeks — *two* monthly payments
...
These tiers correspond to the `Drought Factor` and `Payment Factor` variables in the output data; the `D2 START DATE` … `D4B END` variables record the qualifying drought windows.
